    ...
    async def on_member_join(self, member: Member):
        ...
        if welcome_room:
            ...
            await opt.default_embed(
                header=desc,
                header_icon=True,
                description="ようこそ猿sのばなな農園へ🍌🐵\r🙇公開チャンネルに入る前にお読みだせい",
            )
            opt.add(name="> 規約事項", value="・ 宣伝は必ず許可を取ってください\r・ このサーバの会話の転載はお控えください")
            opt.add(name="> 参加方法", value="規約事項を読み理解したらこのチャットの🗑を押してください")
            await opt.sendEmbed(
                greeting=(member.mention + self.header), seed="w", bottums="🗑"
            )
        else:
